...
quat_node new_quat_node(Quat b){
    quat_node a;
    a.op = (int)b.op;
    a.oprend_1 = b.num1;
    a.oprend_2 = b.num2;
...
void dag_graph::treat(quat_node a) {
    ...
    if(a.op==(int) OP::IF|| a.op==(int) OP::WH){
        dag_node node;
        node.op = a.op;
        node.main=(int) OP::NOP;
        insert_node(node);
        return;
    } else if(a.op==(int) OP::LABEL || a.op==(int) OP::RET || a.op==(int) OP::RETN){
        dag_node node;
        node.op = a.op;
        node.main = a.oprend_1;
        insert_node(node);
        return;
    } else if(a.op==(int) OP::DEF_VAR || a.op==(int)OP::DEF_FUN){
        dag_node node;
        node.op = a.op;
        node.main = a.var;
        insert_node(node);
    ...
    if(a.op!=(int)OP::ASSIGN){
        if(find(a.oprend_1)>=0){
            ...
            if(find(a.oprend_2)>=0){
                ...
                if(n>=0){
                    node_set[n].additional.push_back(a.var);//添加到附加标记
                    hash_map[a.var]=n;
                } else{
                    node.adj.push_back(find(a.oprend_2));
                    insert_node(node);
                    hash_map[a.var] = node_num-1;
                    node_set[hash_map[a.oprend_1]].pre.push_back(node_num-1);
                    node_set[hash_map[a.oprend_2]].pre.push_back(node_num-1);
                }
        ...
        if(find(a.oprend_1)>=0){
            node_set[find(a.oprend_1)].additional.push_back(a.var);
...
block_graph::block_graph(vector<Quat> a) {
    init_quat_list(a);//初始化四元式活动信息
    get_block();//对于四元式进行分块
    get_adj();//对于块构造图
    vector<block_node>::iterator begin=node_set.begin(),end=node_set.end();
    while (begin!=end){
        vector<int>::iterator entry=begin->adjlist.begin(),exit=begin->adjlist.end();
//        cout << "block begin:" <<quat_list[begin->begin].op <<','
//        << quat_list[begin->begin].oprend_1 << ','
//        << quat_list[begin->begin].oprend_2 <<','
//        << quat_list[begin->begin].var << endl;
//        cout << "block end:" <<quat_list[begin->end].op <<','
//             << quat_list[begin->end].oprend_1 << ','
//             << quat_list[begin->end].oprend_2 <<','
//             << quat_list[begin->end].var << endl;
//        for(;entry!=exit;entry++){
//            cout << "next begin:" <<quat_list[entry->begin].op <<','
//                 << quat_list[entry->begin].oprend_1 << ','
//                 << quat_list[entry->begin].oprend_2 <<','
//                 << quat_list[entry->begin].var << endl;
//        }
        begin++;
    }//输出分块结果
}//构造函数
void block_graph::get_adj() {
    ...
    while (now!=end) {//遍历已经分块的图节点
        ...
        if(is_jmp(quat_list[pre->end].op)){//判断是否为跳转语句
...
void block_graph::init_quat_list(vector<Quat> a) {
    for(int i=0;i<a.size();i++){
        quat_list.push_back(new_quat_node(a[i]));
    }
}//初始化四元式序列
//void block_graph::show_block(){
//    for(int i=0;i<node_set.size();i++){
//        cout << "block:" << i+1 << endl;
//        for(int j=node_set[i].begin;j<=node_set[i].end;j++){
//            cout << "(" ;
//            show_op(OP(quat_list[j].op)) ;
//            cout << ',' ;
//            show(quat_list[j].oprend_1);
//            cout << ',';
//            show(quat_list[j].oprend_2);
//            cout << ',';
//            show(quat_list[j].var);
//            cout << ')';
//            cout << endl;
//        }
//    }
...
void block_graph::set_jmp_line() {
    ...
    jmp_line.insert((int)OP::WH);//while
    jmp_line.insert((int)OP::FOR);//for
    jmp_line.insert((int)OP::RET);//RET
    jmp_line.insert((int)OP::RETN);//RETN
...
optimizer::optimizer(vector<Quat>& a) :block_graph(a){
    dag();
    data_flow_analyse();
//    loop_optimize();
    delete_useless_var();
...
void optimizer::generate_quat(const dag_graph &a,vector<block_node>::iterator now) {
    ...
    for(int i=0;i<a.node_num;i++){
        if(a.node_set[i].op==0){
            for(int j=0;j<a.node_set[i].additional.size();j++){
                if(ST.is_define_var(a.node_set[i].additional[j])){
        ...
        } else if(a.node_set[i].op==(int)OP::EI || a.node_set[i].op==(int)OP::EW) {
            new_quat_list[begin].op = a.node_set[i].op;
            new_quat_list[begin].var = (int)OP::NOP;
            new_quat_list[begin].oprend_1 = (int)OP::NOP;
            new_quat_list[begin].oprend_2 = (int)OP::NOP;
            begin++;
        }else if(a.node_set[i].op==(int)OP::LABEL) {
            new_quat_list[begin].op = a.node_set[i].op;
...
void optimizer::show_block(){
    ...
    for(int i=0;i<node_set.size();i++){
        ...
        for(int j=0;j<node_set[i].quat_list.size();j++){
            QL.emplace_back(static_cast<OP>(node_set[i].quat_list[j].op),
                    node_set[i].quat_list[j].oprend_1,
                    node_set[i].quat_list[j].oprend_2,
                            node_set[i].quat_list[j].var);
            cout << "quat_number:" << j+1;
            cout << "  (" ;
            cout << node_set[i].quat_list[j].op;
            cout << ',' ;
    ...
    if(i<node_num-1){
        for(int j=0;j<node_set[i].adjlist.size();j++){
            set_union(LiveOut[i].begin(),LiveOut[i].end(),
                    ...
                    LiveIn[node_set[i].adjlist[j]].end(),inserter(LiveOut[i],LiveOut[i].begin()));
        }
    } else{
        LiveOut[i].clear();
    }
